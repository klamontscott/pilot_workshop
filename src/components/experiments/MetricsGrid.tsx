    ...
    useEffect(() => {
        ...
        function drawTracers(dt: number) {
            for (let i = tracers.length - 1; i >= 0; i--) {
                ...
                const headPos = t.progress
                const tailPos = Math.max(0, t.progress - t.length)
                const tailX = t.x + tailPos
                const headX = t.x + headPos
                if (headX === tailX) continue

                const [r, g, b] = t.color
                const grad = ctx!.createLinearGradient(tailX, t.y, headX, t.y)
                ...
                ctx!.beginPath()
                ctx!.moveTo(tailX, t.y)
                ctx!.lineTo(headX, t.y)
                ctx!.stroke()
            }
        }
    ...
    return (
        <div ref={bannerRef} style={banner}>
            ...
            <div style={row}>
                ...
                <div style={metric}>
                    <div style={number}>41pt</div>
                    <div style={label}>
                        Drop in
                        <br />
                        attrition in 8 months
                    </div>
                </div>
                <div style={metric}>
                    <div style={number}>9/10</div>
                    <div style={label}>
                        New hires did
                        <br />
